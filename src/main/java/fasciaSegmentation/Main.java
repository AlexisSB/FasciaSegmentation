...
public class Main implements PlugIn
{
	...
	private JButton copyRoiButton = null;
	...
	private class CustomWindow extends StackWindow
	{
		...
		CustomWindow(ImagePlus imp)
		{
			...
			setTitle( Main.PLUGIN_NAME + " " + Main.PLUGIN_VERSION );
...
//								//IJ.log("Mouse Released");
		...
		public void addClass()
		{
			...
			exampleList[classNum].setForeground(colors[classNum+1]);
		...
		protected void updateButtonsEnabling()
		{
			...
			else // If the training is not going on
			{
				...
				//copyRoiButton.setEnabled( win.trainingComplete );
	...
	void listSelected(final ItemEvent e, final int i)
	{
		...
		for(int j = 0; j < wekaSegmentation.getNumOfClasses(); j++)
		{
			if (j == i)
			{
				...
				//if the currently selected roi is a point roi, then activate the scissor method.
				if( newRoi instanceof PointRoi){

					IJ.log("Setting colour to " + colors[i]);
	...
	public boolean showCopyDialog(){

		GenericDialogPlus gd = new GenericDialogPlus("Copy Roi");
		//copy class names for labels
		//set all to default true
		boolean[] boolArray = new boolean[numOfClasses];
		for (boolean b : boolArray){
			b = true;
		}
		gd.addCheckboxGroup(numOfClasses, 1,  wekaSegmentation.getClassLabels(), boolArray);
		gd.addNumericField("Copy to Slice : ", displayImage.getCurrentSlice()+1, 0);
		...
		int targetSlice = (int) gd.getNextNumber();
		assert(targetSlice >0 && targetSlice < displayImage.getNSlices());
		for ( int i =0 ; i <numOfClasses; i++) {
			if (gd.getNextBoolean()) {
				for (Roi r : wekaSegmentation.getExamples(i, displayImage.getCurrentSlice())) {
					Roi roi = (Roi) r.clone();
					wekaSegmentation.addExample(i, roi, targetSlice);
				}
			}
		}
	...
	{
		command = "call(\"fasciaSegmentation.Main." + command;
			...
			exec.submit(new Runnable() {
				...
				public void run()
				{
					if(e.getSource() == toggleOverlayButton)
					{
						...
						ImagePlus overlayImage = getLabelImage();
						//first color is background Grey
						// Create overlay LUT
						final byte[] red = new byte[ 256 ];
						final byte[] green = new byte[ 256 ];
						final byte[] blue = new byte[ 256 ];
						...
						// assign colors to classes
						Color[] labelcolors = new Color[ WekaSegmentation.MAX_NUM_CLASSES ];
						labelcolors[0] = new Color(179,204,204);
						...
						{
							labelcolors[ i ] = Color.getHSBColor(hue, saturation, 1);
						...
						{
							//IJ.log("i = " + i + " color index = " + colorIndex);
							red[i] = (byte) labelcolors[ i ].getRed();
							green[i] = (byte) labelcolors[ i ].getGreen();
							blue[i] = (byte) labelcolors[ i ].getBlue();
						}

						LUT labelLUT = new LUT(red, green, blue);
