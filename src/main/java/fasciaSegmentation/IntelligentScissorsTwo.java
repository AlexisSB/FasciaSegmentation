...
public class IntelligentScissorsTwo {
    ...
    private ArrayList<ArrayList<Point>> findEdgeAnchorPoints() {
        ...
        for(Point p : userSelectedPoints){
            ...
            double slope;
            if(pointIndex == 0){
                slope = findNormalDirection(centreLinePath.subList(0,2));
            }else if( pointIndex == userSelectedPoints.size()-1){
                slope = findNormalDirection(centreLinePath.subList(pointIndex-2, pointIndex));
            }else{
                slope = findNormalDirection(centreLinePath.subList(pointIndex-1, pointIndex+1));
            }
            ...
            //Add the point to the edge anchors list
            edgeAnchorPoints.add(newOppositeEdgePoint);
